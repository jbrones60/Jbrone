...
const MEMBER_OPTIONS = ['Ravi', 'Priya', 'Suresh'];
  ...
  return (
    <div style={s.wrap}>
      <table style={s.table}>
        <thead>
          <tr>
            {['#', 'Name', 'Category', 'Phone', 'Website', 'Assigned', 'Priority', 'Status'].map(h => (
          ...
          {slice.map((lead, i) => (
            <tr key={lead.id} style={s.trHover} onClick={() => onSelectLead(lead)}>
              ...
              <td style={s.td} onClick={e => e.stopPropagation()}>
                <select style={s.select} value={lead.assigned_to} onChange={e => handleInlineChange(lead, 'assigned_to', e.target.value)}>
