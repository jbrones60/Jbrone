      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
        {KPI_CONFIG.map(({ key, label, color, derive }) => (
          <div key={key} style={{ background: '#111827', borderLeft: `4px solid ${color}`, borderRadius: 8, padding: '18px 20px' }}>
            ...
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: '#64748b', marginTop: 6 }}>{label}</div>
          </div>
        ))}
      </div>
              ...
              return (
                <div key={r.category} style={{ background: '#111827', borderLeft: `4px solid ${color}`, borderRadius: 8, padding: '12px 16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
                    ...
                    <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 11, color: '#64748b' }}>{r.called}/{r.total} called · {pct}%</span>
                  </div>
            ...
            {stats.byStatus.sort((a, b) => b.count - a.count).map(r => {
              ...
              return (
                <div key={r.status} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#111827', borderRadius: 8, padding: '10px 14px' }}>
                  <span style={{ background: color + '22', color, border: `1px solid ${color}44`, borderRadius: 20, padding: '3px 12px', fontFamily: 'DM Mono, monospace', fontSize: 12 }}>
                    {r.status}
                  </span>
                  <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 14, fontWeight: 600, color: '#f1f5f9' }}>{r.count}</span>
                </div>
              );
            })}
          </div>
              ...
              return (
                <div key={r.lost_reason} style={{ background: '#111827', borderRadius: 8, padding: '12px 16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: '#94a3b8' }}>{r.lost_reason}</span>
                    <span style={{ fontFamily: 'DM Mono, monospace', fontSize: 12, color: '#64748b' }}>{r.count} · {pct}%</span>
                  </div>
                  <div style={{ height: 4, background: '#1e293b', borderRadius: 2 }}>
